...
5. Set up a BEAUTi template. There is a partial solution but some things need
   work:
   - Currently occurrence times must be specified manually
   - UpDown operators need to be specified manually for the parameters that need
     them
   - The schedule for catastrophes must be specified manually
...
There is a partial template for Beauti in =templates/TimTam.xml=. It has
limitations described in the TODO list above.
