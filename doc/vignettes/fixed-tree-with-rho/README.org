...
1. copy the Newick string representation of the tree into a =beast.util.TreeParser=
   and make sure that the TimTam is referencing this,
2. include the catastrophe time, 0.0, into the =BackwardsSchedule= slot of the
   TimTam (it takes this value by default but it is nice to double check).
...
4. and finally delete references to anything relating to sequences and tree
   estimation.
5. double check the log writes to =fixed-tree-with-rho-01.log=,
6. save the result to =fixed-tree-with-rho-01.xml=.
