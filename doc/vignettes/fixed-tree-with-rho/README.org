...
3. Set up the prior in the *Priors* tab
   ...
   * Set the priors for the becoming uninfectious rate, reproductive number and
     sampling proportion as two gamma distributions and a beta distribution with
     parameters \((4,0.25)\), \((3,0.5)\), and \((2,6)\) respectively.
4. Save this as a suitable XML file, e.g., =bdsky-serial-2022-03-11.xml=
   * (Optional:) use a tool such as =js-beautify= to format the XML.
5. Use the =xml-munge-bdsky.R= script to do some boring edits to obtain
   =bdsky-serial-2022-03-11-edited.xml= and =bdsky-2022-03-11-edited.xml=, the
