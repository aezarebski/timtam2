#+title: Estimated Tree Example Without Rho
...
#+begin_src sh
  ./ape-sim.R --seed 1 -p my-params.json -o out --duration 4.0 -v --simulate-sequences --make-plots
#+end_src
...
and write them to a text file. Note that unlike in the examples where there is a
\(\rho\)-sample we treat the final tip-time as the present.
...
#+begin_src R
  ...
  last_sample_time <- max(x[x$event == "sampling", "time"]) + root_length
  bwd_occurrence_times <- last_sample_time - occurrence_times
...
- include a =BackwardsPointProcess= input using the occurrence times from the
  simulation,
