...
After seeing that this analysis produces something sensible, we can make a copy
in =out/analysis-timtam.xml= and use that as a starting point for an analysis
using TimTam. The file =out/ape-sim-event-times.csv= contains the times of all the
observations in the simulation. This needs to be put into the XML along with a
list of the taxa it includes and the occurrence times. The following snippet
helps with formatting the occurrence times for inclusion in the XML.
