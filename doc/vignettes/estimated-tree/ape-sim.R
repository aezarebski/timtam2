...
parser$add_argument(
         ...
         action = "store_true",
         default = FALSE,
         help = "Simulate sequences for each of the leaves of the reconstructed tree. This makes use of the simSeq function from phangorn.")
...
run_conditioned_simulation <- function(params, is_verbose) {
  ...
  if (params$simulate_sequences) {
    result$seq_sim <- sequence_simulation(result$reconstructed_tree, is_verbose)
  }
  return(result)
}

sequence_simulation <- function(tr, is_verbose) {
  if (is_verbose) {
    cat("running the additional simulation of sequences for reconstructed tree...\n")
  }
  return(simSeq(tr))
}
